const { app, BrowserWindow, BrowserView, ipcMain, Menu, clipboard, session, shell } = require('electron');
...
function loadState() {
  try {
    ...
    return {
      ...
      instances,
      activeId: raw.activeId || instances[0].instanceId,
      order: Array.isArray(raw.order) ? raw.order : instances.map((i) => i.instanceId),
    };
  } catch {
    return seedState();
  }
...
function loadLock() {
  try {
    ...
    return { enabled: false, salt: null, hash: null };
  }
}
...
function createViewForInstance(instance) {
  ...
  attachViewBehaviors(instance, view);
  if (isSafeHttpUrl(instance.url)) view.webContents.loadURL(instance.url);
  views[instance.instanceId] = view;
  return view;
}

function destroyView(instanceId) {
  const view = views[instanceId];
  if (!view) return;
  try {
    if (mainWindow && mainWindow.getBrowserView() === view) mainWindow.setBrowserView(null);
...
function switchTo(instanceId) {
  if (!instanceId || overlayOpen) {
    state.activeId = instanceId || state.activeId;
    saveStateSoon();
    sendUi();
    return;
  }
...
function createWindow() {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    ...
    mainWindow.webContents.send('boot', {
      needsLock: !!(lock.enabled && lock.hash),
      version: app.getVersion(),
    });
    sendUi();
  });
...
function setupIpc() {
  ...
  ipcMain.on('add-instance', (_e, payload) => {
    ...
    const instance = {
      instanceId,
      appId,
      label: (payload.label && String(payload.label).trim()) || name,
      color,
      url,
      folderId: payload.folderId || null,
    };
    state.instances.push(instance);
    if (!state.order.includes(instanceId)) state.order.push(instanceId);
    saveStateSoon();
    overlayOpen = false;
    switchTo(instanceId);
  });
  ipcMain.on('remove-instance', async (_e, instanceId) => {
    ...
    if (state.activeId === instanceId) {
      state.activeId = state.instances[0] ? state.instances[0].instanceId : null;
    }
    saveStateSoon();
  ...
  ipcMain.on('rename-instance', (_e, { instanceId, label }) => {
    ...
    inst.label = String(label).slice(0, 40);
    saveStateSoon();
    sendUi();
  });
  ...
  ipcMain.on('move-to-folder', (_e, { instanceId, folderId }) => {
    ...
    if (folderId) {
      const f = state.folders.find((x) => x.id === folderId);
      if (f && !f.itemIds.includes(instanceId)) f.itemIds.push(instanceId);
    }
    saveStateSoon();
    sendUi();
  });
  ...
  ipcMain.handle('disable-password', (_e, current) => {
    ...
    if (hashPassword(current || '', lock.salt) !== lock.hash) return { ok: false, error: 'رمز اشتباه است' };
